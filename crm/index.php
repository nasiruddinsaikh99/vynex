...
$secret = '86e1b976caa57ecec3b7e9f5c3ae073e';
...
// Helper: build URL with secret parameter
function url_with_secret($params = [], $secret)
{
	// Ensure secret parameter is always included
	$allParams = $params;
	$allParams[$secret] = $allParams[$secret] ?? '';
...
// Email status filter - defaults to 'valid' if not specified
$flt_email_status = isset($_GET['email_status']) ? trim($_GET['email_status']) : 'valid';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
	...
	if ($action === 'create') {
		...
		if ($email === '') {
			header("Location: " . url_with_secret(['err' => 'Email is required'], $secret));
			exit;
		}
		...
		header("Location: " . url_with_secret(['id' => $nextId, 'ok' => 'Created'], $secret));
		exit;
	}
	...
	if ($action === 'update' && isset($_POST['id'])) {
		...
		header("Location: " . url_with_secret(['id' => $id, 'ok' => 'Updated'], $secret));
		exit;
	}
	...
	if ($action === 'update_status' && isset($_POST['id'])) {
		...
		header("Location: " . url_with_secret(['id' => $id, 'ok' => 'Status Saved'], $secret));
		exit;
	}
	...
	if ($action === 'save_notes' && isset($_POST['id'])) {
		...
		header("Location: " . url_with_secret(['id' => $id, 'ok' => 'Notes Saved'], $secret));
		exit;
	}
}
...
if ($flt_email_status !== '' && $flt_email_status !== 'all') {
	$where[] = "email_status = ?";
	$params[] = $flt_email_status;
	$types .= "s";
}
...
function compose_prompt(array $r): string
{
	...
	$lines[] = "- Website: " . trim(($r['website_name'] ?? '') ?: 'N/A') . " (URL: " . trim(($r['url'] ?: $r['url'] ?: 'N/A')) . ")";
	$lines[] = "- Country: " . trim(($r['country'] ?? '') ?: 'N/A');
	$lines[] = "- Industry: " . trim(($r['industry'] ?? '') ?: 'N/A');
	$lines[] = "- Website Members Profession/Trade: " . trim(($r['profession'] ?? '') ?: (($r['trade'] ?? '') ?: 'N/A'));
	$lines[] = "";
	$lines[] = "Context:";
	$lines[] = "";
	return implode("\n", $lines);
		...
		<div class="flex gap-4">
			...
			<aside class="basis-3/10 min-w-[260px] bg-white rounded-2xl shadow p-4 h-[78vh] overflow-auto">
				<form class="space-y-3 mb-4" method="get">
					<input type="hidden" name="<?= e($secret) ?>" value="" />
					<div>
					...
					<div>
						<select name="email_status" class="w-full border rounded-xl px-3 py-2">
							<option value="all" <?= $flt_email_status === 'all' ? 'selected' : '' ?>>All Email Status</option>
							<option value="valid" <?= $flt_email_status === 'valid' ? 'selected' : '' ?>>Valid</option>
							<option value="invalid" <?= $flt_email_status === 'invalid' ? 'selected' : '' ?>>Invalid</option>
							<option value="unknown" <?= $flt_email_status === 'unknown' ? 'selected' : '' ?>>Unknown</option>
						</select>
					</div>
				...
				<ul class="space-y-2">
					...
					?>
						<li>
							<a href="<?= e(url_with_secret(array_merge($_GET, ['id' => $r['id']]), $secret)) ?>"
				...
				<?php if ($totalPages > 1): ?>
					<div class="mt-4 flex items-center justify-between text-sm">
						...
						$qs = $_GET;
						$qs['page'] = $prev;
						$prevUrl = url_with_secret($qs, $secret);
						$qs['page'] = $next;
						$nextUrl = url_with_secret($qs, $secret);
						?>
				...
				?>
					<div class="flex items-start justify-between gap-4">
						...
						<div class="text-right">
							<?php if ($sel['url'] || $sel['secure_url']): ?>
								<a target="_blank" href="http://<?= e($sel['url']) ?>"
					...
					<details class="mt-8">
						...
						<form method="post" class="mt-4 grid grid-cols-1 md:grid-cols-3 gap-4 bg-white p-4 border rounded-2xl">
							...
							function renderInput($label, $name, $val, $ph = '')
							{
								echo '
