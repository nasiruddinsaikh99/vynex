<?php require_once('templates/header.php') ?>
    <main>



        <section class="categories-area section-padding">
            <div class="container">
                <div class="row justify-content-center">
                    <div class="col-xl-6 col-lg-6 col-md-10">

                        <div class="section-tittle text-center mb-35">
                            <span>Our Services</span>
                            <h2>IT solutions for startup and enterprises</h2>
                        </div>
                    </div>
                </div>
                <div class="row">
                    <div class="col-lg-4 col-md-6 col-sm-6">
                        <div class="single-cat text-center mb-50">
                            <div class="cat-icon">
                                <img src="assets/img/icon/ai.svg" alt>
                            </div>
                            <div class="cat-cap">
                                <h5><a href="services.php">AI-Powered Services</a></h5>
                                <p>Software solutions that automate, optimize, and transform business processes</p>
                            </div>
                        </div>
                    </div>
                    <div class="col-lg-4 col-md-6 col-sm-6">
                        <div class="single-cat text-center mb-50">
                            <div class="cat-icon">
                                <img src="assets/img/icon/stack3.svg" alt>
                            </div>
                            <div class="cat-cap">
                                <h5><a href="services.php">Custom Software Solutions</a></h5>
                                <p>Tailored software development to meet unique business needs, goals, and processes</p>
                            </div>
                        </div>
                    </div>
                    <div class="col-lg-4 col-md-6 col-sm-6">
                        <div class="single-cat text-center mb-50">
                            <div class="cat-icon">
                                <img src="assets/img/icon/mobile.svg" alt>
                            </div>
                            <div class="cat-cap">
                                <h5><a href="services.php">Mobile App Development</a></h5>
                                <p>Creating engaging, user-friendly mobile apps for iOS and Android devices</p>
                            </div>
                        </div>
                    </div>
                    
                </div>
            </div>
        </section>


        <section class="about-area2 section-padding gray-bg ">
            <div class="container">
                <div class="row align-items-center justify-content-between">
                    <div class="col-xl-5 col-lg-5 col-md-10">

                        <div class="about-img">
                            <img src="assets/img/gallery/about1.jpg" alt>
                        </div>
                        <div class="ratting-point">
                            <div class="features-ratting">
                                <img src="assets/img/icon/notification.svg" alt>
                            </div>
                            <div class="features-caption">
                                <h3>Enhance Customer Experience</h3>
                                <p>Deliver personalized, seamless interactions</p>
                            </div>
                        </div>
                    </div>
                    <div class="col-xl-5 col-lg-6 col-md-10">
                        <div class="about-caption mb-50">

                            <div class="section-tittle mb-50">
                                <h2>Grow your<br> business fast.</h2>
                            </div>
                            <div class="single-offers mb-20">
                                <div class="offers-cap">
                                    <img src="assets/img/icon/notification1.svg" alt>
                                    <h3><a href="services.php">AI-Powered Operations</a></h3>
                                    <p>Automate tasks, predict trends with AI & machine learning.</p>
                                </div>
                            </div>
                            <div class="single-offers">
                                <div class="offers-cap">
                                    <img src="assets/img/icon/notification1.svg" alt>
                                    <h3><a href="services.php">Chatbot Customer Support</a></h3>
                                    <p>Naxly bring the power of data science and artificial intelligence to every business.</p>
                                </div>
                            </div>
                            <a href="about.php" class="btn mt-20">Learn More</a>
                        </div>
                    </div>
                </div>
            </div>
        </section>


        <section class="services-area2 section-padding2">
            <div class="container">
                <div class="row align-items-center justify-content-between">
                    <div class="col-sm-11">

                        <div class="section-tittle section-tittle2 mb-30">
                            <span>Industries We Serve</span>
                            <h2>IT services customized for your industry</h2>
                        </div>
                    </div>
                </div>
                
                <div class="row">
                    <div class="col-sm-11">
                        <div class="section-tittle section-tittle2 mb-55">
                            <p>At Syntrex, we believe that innovation and technology can transform any industry. That's why we don't limit ourselves to specific sectors. Whether you're in healthcare, finance, e-commerce, or any other field, our custom software solutions and expertise can help you achieve your goals. We're industry-agnostic, meaning we can adapt our services to meet the unique needs of your business, regardless of the industry you're in. Let's work together to drive success and growth, no matter what industry you're in.</p>
                        </div>
                    </div>
                </div>

                <div class="row justify-content-center">
                    <div class="col-xl-12">
                        <div class="section-tittle section-tittle3 pt-20">
                            <p>Need a Technology support? <a href="contact.php">Get Started Now</a></p>
                        </div>
                    </div>
                </div>
            </div>
        </section>


        <div class="count-down-area section-padding border-bottom">
            <div class="container">
                <div class="row justify-content-center">
                    <div class="col-xl-8 col-lg-9  col-md-12">

                        <div class="section-tittle text-center mb-60">
                            <h2>Syntrex - Partner for Your Business </h2>
                            <p class="mb-55">Experience the power of a reliable partnership. At Syntrex, we're dedicated to delivering exceptional solutions, fostering long-term relationships, and driving business growth through expertise, innovation, and unwavering commitment.</p>
                            <a href="contact.php" class="border-btn">Request a Quote</a>
                        </div>
                    </div>
                </div>
                <div class="row justify-content-center ">
                    <div class="col-lg-12 col-md-12">
                        <div class="count-down-wrapper">
                            <div class="row justify-content-between">
                                <div class="col-xl-3 col-lg-4 col-md-6 col-sm-6">

                                    <div class="single mb-30">
                                        <div class="single-counter text-center">
                                            <span class="counter ">450</span>
                                            <p class>+</p>
                                        </div>
                                        <div class="pera-count  text-center">
                                            <p>Successful Projects Delivered</p>
                                        </div>
                                    </div>
                                </div>
                                <div class="col-xl-3 col-lg-4 col-md-6 col-sm-6">
                                    <div class="single mb-30">

                                        <div class="single-counter text-center">
                                            <span class="counter ">180</span>
                                            <p class>+</p>
                                        </div>
                                        <div class="pera-count text-center">
                                            <p>Happy Clients Worldwide</p>
                                        </div>
                                    </div>
                                </div>
                                <div class="col-xl-3 col-lg-4 col-md-6 col-sm-6">
                                    <div class="single mb-30">

                                        <div class="single-counter text-center">
                                            <span class="counter ">94</span>
                                            <p class>%</p>
                                        </div>
                                        <div class="pera-count text-center">
                                            <p>Client Retention Rate</p>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>

    </main>
    <?php require_once('templates/footer.php') ?>